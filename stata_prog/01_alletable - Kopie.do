...
mvdecode m1202, mv(-1)
glo tab_dir "D:\oCloud\Home-Cloud\Lehre\BIBB\StataBIBB3"
glo tab_dir "D:\oCloud\Home-Cloud\Lehre\BIBB\StataBIBBtex"
...
esttab using "${tab_dir}/desc.rtf", ///
	cells("sum(fmt(%13.0fc)) mean(fmt(%13.2fc)) sd(fmt(%13.2fc)) min max count") nonumber ///
	nomtitle nonote noobs label collabels("Sum" "Mean" "SD" "Min" "Max" "N") ///
	replace
   	
esttab using "${tab_dir}/desc.tex", ///
	cells("sum(fmt(%13.0fc)) mean(fmt(%13.2fc)) sd(fmt(%13.2fc)) min max count") nonumber ///
	nomtitle nonote noobs label collabels("Sum" "Mean" "SD" "Min" "Max" "N")  ///
	coeflabel(F518_SUF "Bruttoverdienst") ///
	replace booktabs
...
* help correlate
esttab
esttab ., notitle unstack compress noobs nomtitle nonumber
